...
export default function AdminPanel({ content, onContentChange, onLock }) {
  ...
  const [aboutPrimary, setAboutPrimary] = useState(() => (content.profile?.aboutPrimary || "我叫杨坤，是一名专注内容叙事与视觉表达的 AI 短剧剪辑师。熟悉真人短剧的粗剪、精剪与节奏把控，也能独立完成 AI 漫剧从小说改写、剧本分镜、资产图建立、视频生成到剪辑成片的完整流程。").replaceAll("李万民", "杨坤"));
  ...
  const [displayName, setDisplayName] = useState(() => content.profile?.displayName === "李万民" ? "杨坤" : (content.profile?.displayName || "杨坤"));
  const [heroRole, setHeroRole] = useState(() => content.profile?.heroRole || "剪辑师 / AI设计师 / AI漫剧");
  const [footerCopyright, setFooterCopyright] = useState(() => (content.profile?.footerCopyright || "© 2024—2026 杨坤 · 保留所有权利").replaceAll("李万民", "杨坤"));
  ...
  useEffect(() => {
    ...
    setAboutPrimary((profile.aboutPrimary || "我叫杨坤，是一名专注内容叙事与视觉表达的 AI 短剧剪辑师。熟悉真人短剧的粗剪、精剪与节奏把控，也能独立完成 AI 漫剧从小说改写、剧本分镜、资产图建立、视频生成到剪辑成片的完整流程。").replaceAll("李万民", "杨坤"));
    ...
    setDisplayName(profile.displayName === "李万民" ? "杨坤" : (profile.displayName || "杨坤"));
  ...
  return <aside className="admin-drawer" aria-label="内容管理"><header><strong>内容管理</strong><button type="button" aria-label="锁定并关闭管理" onClick={onLock}><X size={20} /></button></header><div className="admin-scroll">
    ...
    <section><h3>腾讯云 COS 授权</h3><input type="password" placeholder="SecretId（建议使用仅限此桶写入的子账号）" value={config.secretId} onChange={(e) => setConfig({ ...config, secretId: e.target.value })}/><input type="password" placeholder="SecretKey" value={config.secretKey} onChange={(e) => setConfig({ ...config, secretKey: e.target.value })}/><button type="button" onClick={saveConfig}>保存授权</button><p className="admin-note">密钥仅保存在当前浏览器，不会提交到 GitHub。</p></section>
